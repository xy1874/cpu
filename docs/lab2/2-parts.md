...
&emsp;&emsp;在图3-5中，`pc_sel`信号来源于控制单元，用于选择PC的新值。
...
!!! info "IROM配置说明 :mega:"
    &emsp;&emsp;本实验需要创建64KB大小的IROM，因此IROM共包含16384个数据单元 (数据位宽为32位)，对应地址线14根。

&emsp;&emsp;配置完成后，依次点击“OK”按钮和“Generate”按钮。此时，Vivado将在当前工程中生成一个名为`prgrom.xci`的IROM模块。

&emsp;&emsp;需要注意的是，此时的IROM尚未初始化，其内部不含有任何有效数据。为了让CPU运行汇编程序，我们需要将汇编器生成的机器码导入到IROM当中。

&emsp;&emsp;Distributed Memory Generator的IP核支持通过`.coe文件`导入初始数据。`.coe文件`的语法形如图3-10所示。
...
!!! info "IP核编址方式说明 :mega:"
    &emsp;&emsp;以图3-10所示的.coe文件为例，0地址对应第1个数据0x3c01ffff，1地址对应第2个数据0x343cf000，依此类推。
...
``` Verilog
 ...
 5|    // 64KB IROM
 6|    prgrom U0_irom (
 7|        .a      (pc_i[15:2]),   // input wire [13:0] a
 8|        .spo    (instruction)   // output wire [31:0] spo
 9|    );
10|
11|    ......
12|endmodule
...
``` Verilog
 1|module dmemory32 (
 2|    input           clk_i,
 3|    input   [31:0]  addr_i,     // 来源于执行单元计算出的访存地址
 4|    output  [31:0]  rd_data_o,  // 从存储器中获得的数据
 5|    input           memwr_i,    // 来自控制单元的写使能信号
 6|    input   [31:0]  wr_data_i   // 来自译码单元的rd_data2 (S型指令的rs2寄存器的值)
 7|);
 8|    ......
 9|
10|endmodule
...
1|    // 64KB DRAM
2|    dram U_dram (
3|        .clk    (clk_i),            // input wire clka
4|        .a      (addr_i[15:2]),     // input wire [13:0] addra
5|        .spo    (rd_data_o),        // output wire [31:0] douta
6|        .we     (memwr_i),          // input wire [0:0] wea
7|        .d      (wr_data_i)         // input wire [31:0] dina
8|    );
```
